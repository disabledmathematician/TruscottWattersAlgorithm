...
def Charles():
#	plt.figure(0, dpi=145, figsize=[14, 7])
...
#	plt.hist(df['Max Daily Rain'], edgecolor='crimson', bins=range(int(df["Max Daily Rain"].min()), int(df['Max Daily Rain'].max())))
#	ax4.title.set_text("Highest Volume of Daily Rainfall Byron Bay")
#	ax4.set_xlabel("Volume")
#	ax4.set_ylabel("Frequency")
#	plt.show()
#	plt.savefig('all.png')
	plt.hist(df['Max Daily Rain'].round(), edgecolor='crimson', cumulative=True, bins=range(int(df["Max Daily Rain"].round().min()), int(df['Max Daily Rain'].round().max())))
	plt.xticks(range(int(df["Max Daily Rain"].round().min()), int(df['Max Daily Rain'].round().max()), 25))
#	plt.savefig("yrsbyronrain.png")
	plt.show()
Charles()